<?php
	...
	function deleteAll($path)
	{
		global $mySessionFront;
	      ...
	      foreach(glob($path.'*.*') as $v)
	      {
	      	...
	      	$profile = strstr($v, $mySessionFront->user['Username'].".");
	      	if($profile != false)
	      	{
	      		unlink($v);
	      	}
	...
	{
		global $mySessionFront;
		if($mySessionFront->user['userRole']=='SA') {
			if($mySessionFront->user['rights'] && $mySessionFront->user['rights']!='') {
				if(strstr($mySessionFront->user['rights'],$rolename)) {
					return true;
				}
			} 
		} 
	...
	function getsenior_data($id)
	{
		...
		if(is_array($staterecord) && count($staterecord) > 0 && $staterecord[0]['employeeId']!=0) {
			$qry2 = "SELECT Id, profileId "
							." FROM rbi_agent as a"
							." Where Id='".$staterecord[0]['employeeId']."' ";
			$staterecord2 = $db->runQuery($qry2);
		} else {
			$staterecord2[0]['FrontUserId']=''; 
