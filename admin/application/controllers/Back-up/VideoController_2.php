...
class VideoController extends AppController
{
	...
	if ($objRequest->isPost()) {
		...
		if($form->isValid($formData)){
			...
			$db->save('jok_subcatdetail',$Data); 
			$mySession->sucMsg = "VIDEO ADDED";
			$this->_redirect('Video/add');
			
			$qry="SELECT MAX(CatDetailSno) AS my_cust_ac_no FROM jok_subcatdetail";
			$result=$db->runQuery($qry);
			//$maxid=5000;
			if(is_array($result) && count($result) >0)
			{
				if($result[0]['my_cust_ac_no'] > 0) {
			...
			$Data['VideoImage'] = $image;
			 prd($Data);
			$db->save('jok_subcatdetail',$Data); 
			$mySession->sucMsg = "NEW VIDEO ADDED";
			$this->_redirect('Video/add');
