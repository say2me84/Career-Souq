...
FROM
tbl_job_categories
LEFT JOIN tbl_jobs ON tbl_job_categories.cat_id = tbl_jobs.job_category WHERE cat_id = 1 ORDER BY category_banner";
			//prd($Home_Cat_Img_Qry);
			$Home_Cat_Data = $db->runQuery("$Home_Cat_Img_Qry");
				
				//prd($Home_Cat_Data);
			$this->view->Home_Cat_Img=$Home_Cat_Data[0];
//================================================================== Loding First Category Job =================================================== //
		$qry="SELECT
tbl_jobs.id,
tbl_jobs.job_title,
...
WHERE job_category ='1' ORDER BY tbl_jobs.job_posted_on DESC limit 0,2";
			//prd($qry);
			...
			if(isset($formData['ptype'])) {
				...
				if(is_array($result)){	
					if($result['user_is_blocked']==1){	
						...
						$mySessionFront->user['FirstLastName']=$result['user_fname'].' '.$result['user_lname'];
...
//----------------------------------- Sending Mail to User -----------------------------------------------------------		
	...
	public function fbloginAction(){
		...
		//$mySessionFront->user['FrontUserId'] = $result['user_id'];
		//$mySessionFront->user['FrontUserId'] = $result['user_id'];
		$app_id = "363211403814528";//change this
		$redirect_url = "http://localhost/myfb/callback.php"; //change this

$dialog_url = "https://www.facebook.com/dialog/oauth?client_id=".$app_id ."&redirect_uri=" . urlencode($redirect_url) . "&state="
