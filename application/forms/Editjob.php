<?php
class Form_Editjob extends Zend_Form
{
	public function __construct($id = NULL)
	{
		...
		if($id!="")

			{
				...
				$Job_Title_Value=$formData[0]['job_title'];
				$job_keywords_Value=$formData[0]['job_keywords'];
				$job_category_Value=$formData[0]['job_category'];
				$job_sub_category_Value=$formData[0]['job_sub_category'];
				$Description_Value=$formData[0]['job_description'];
				$job_responsibilities_Value=$formData[0]['job_responsibilities'];
				$job_skills_required_Value=$formData[0]['job_skills_required'];
				$job_country_Value=$formData[0]['job_country'];
				$job_city_Value=$formData[0]['job_city'];
				$job_employment_type_Value=$formData[0]['job_employment_type'];
				$job_type_Value=$formData[0]['job_type'];
				$job_career_level_Value=$formData[0]['job_career_level'];
				$job_education_Value=$formData[0]['job_education'];
				$job_experience_Value=$formData[0]['job_experience'];
				$job_travel_required_Value=$formData[0]['job_travel_required'];
				$job_relocation_Value=$formData[0]['job_relocation'];
				$job_role_Value=$formData[0]['job_role'];
				$Sallary_Value=$formData[0]['job_sallary'];
				$Email_Value=$formData[0]['job_email'];
				$job_phone_number_Value=$formData[0]['job_phone_number'];
				$job_contact_name_Value=$formData[0]['job_contact_name'];
				$job_fax_Value=$formData[0]['job_fax'];
			}
		...
		->setValue(@$job_sub_category_Value);
		...
		$Responsibilities->setRequired(true)
				...
				->setAttrib("class", "textarea ckeditor")
				->setValue(@$job_responsibilities_Value)
				->addDecorator('Errors', array('class'=>'errormsg'));
		...
		$Skills_Required->setRequired(true)
				...
				->setAttrib("class", "textarea ckeditor")
				->setValue(@$job_skills_required_Value)
				->addDecorator('Errors', array('class'=>'errormsg'));
...
/*---------------------------------------- Employment Type ----------------------------------------------------*/
		...
		$Employment_Type->addMultiOptions($EmploymentArr)		
			...
			->addDecorator('Errors', array('class'=>'errormsg'))
			->setValue(@$job_employment_type_Value)
			->setAttrib("class","inputbox")
			->setAttrib("style","width:150px;");
...
/*---------------------------------------- Career_Level ----------------------------------------------------*/
		...
		$Career_Level->addMultiOptions($Career_LevelArr)		
			...
			->addDecorator('Errors', array('class'=>'errormsg'))
			->setAttrib("class","inputbox")
			->setValue(@$job_career_level_Value)
			->setAttrib("style","width:150px;");
...
/*---------------------------------------- Experience ----------------------------------------------------*/		
		...
		$Travel_Required->setRequired(true)
				...
				->setAttrib("class","inputbox")
			->setAttrib("style","width:150px;")
			->setValue(@$job_travel_required_Value)
				->addDecorator('Errors', array('class'=>'errormsg'));
...
/*---------------------------------------- job_role ----------------------------------------------------*/
		...
		$Phone_Number->setRequired(true)
				...
				->addDecorator('Errors', array('class'=>'errormsg'))
				->setValue(@$job_phone_number_Value)
				//->setAttrib("onkeypress","return checknumber(event)")
				->setAttrib("class","inputbox");
		...
		$Contact_Name->setRequired(true)
				...
				->addDecorator('Errors', array('class'=>'errormsg'))
				->setValue(@$job_contact_name_Value)
				->setAttrib("class","inputbox");
