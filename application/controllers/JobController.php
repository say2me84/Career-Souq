...
class JobController extends Zend_Controller_Action
{
	...
	{	
		...
		if (!empty($arr['job_keywords'])) {
			$wherecondition .= " and (job_title like '%" .$arr['job_keywords']."%' or job_keywords like '%".$arr['job_keywords']."%' )";
		}
...
//================ Edit Save Job ================
	...
	{	
		...
		$this->view->pagetitle = 'Edit Save Job';
		$Company_Qry="SELECT
tbl_countries.country_name,
tbl_career_levels.career_level_title,
tbl_nationalities.nation_title,
...
//=========================================   Resumes  ==========================//
		...
		//prd($data);
				$condition = "id='".$arr['id']."'";
				$db->modify('tbl_jobs',$data,$condition);
		...
		public function jobdetailAction(){
			...
			if($arr['id']) {
				$qry="SELECT
	tbl_jobs.id,
	tbl_jobs.user_id as company_id,
	...
	WHERE tbl_jobs.id='".$arr['id']."'";
		...
		public function preferredjobAction(){
			global $mySessionFront;
			$db=new Db();
	...
	//============================================================ SAVE AND APPLY JOBS =========================================================
		public function jobapplyAction(){
			...
			$arr1=$this->getRequest()->getParams('job_id');
			$arr2=$this->getRequest()->getParams('job_id');
			//prd($arr2['job_id']);
			
			$dtr=explode('&',$arr['job_id']);
			$job_id = $arr['job_id'] = $dtr[0];
			//prd($job_id);
			//echo 'Job Id ='.$job_id; exit;
			$dtr1=explode('&',$arr1['job_id']);
			$company_id = $arr1['job_id'] = $dtr1[1];
			//echo 'Employee Id ='.$company_id; exit;
			
			$dtr2=explode('&',$arr2['job_id']);
			$job_name = $arr2['job_id'] = $dtr2[2];
			//echo 'Job Name ='.$job_name; exit;
			
			$modelobj = new Model_Mainmodel();
$result=$modelobj->runThisQuery("select * from tbl_applied_jobs where user_id='".$mySessionFront->user['FrontUserId']."' and job_id='".$job_id."'");
			//prd($result);
		...
		if($arr['job_id']) {
				...
				$data1='';
				$data1['to']= $company_id;
				$data1['from']= $mySessionFront->user['FrontUserId'];
				$data1['subject']=$job_name;
				$data1['message']=$mySessionFront->user['FirstLastName'].'&nbsp;'.'has applied for the job'.'&nbsp;'.$job_name;
				$data1['sent_on']=date('Y-m-d');
				$data1['is_read']='0';
				
				$modelobj = new Model_Mainmodel();
				$insid = $modelobj->insertThis('tbl_notifications',$data1);
		...
		public function prefjobapplyAction(){
			global $mySessionFront;
			$db=new Db();	
...
$result=$modelobj->runThisQuery("select * from tbl_applied_jobs where user_id='".$mySessionFront->user['FrontUserId']."' and job_id='".$job_id."'");
			...
			if(is_array($result) && count($result) > 0)
			{
				...
				//echo "You already applied this job.";
				$mySessionFront->errorMsg="You already applied this job.";
				$this->_redirect("job/preferredjob/id/".$arr['job_id']."");
				//$this->_redirect('index/login');
			}else{ 
		...
		public function prefjobsaveAction(){
			global $mySessionFront;
			//$this->_helper->layout()->setLayout('ajaxlayout');	
