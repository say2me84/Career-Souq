...
class ZoneController extends Zend_Controller_Action
{
	...
	{		
		...
		$json .= "rows: [";
		$rc = false;
		if(isset($roles[0]) && $roles[0]['country_id']!="")
		{
		$i=1;
		...
		{			
			if ($rc) $json .= ",";
			$json .= "\n{";
			$json .= "id:'".$row['country_id']."',";
			$json .= "cell:['".$i."'";
			$json .= ",'".$row['country_name']."'";
			$json .= ",'<a href=\"".APPLICATION_URL."Zone/edit/id/".$row['country_id']."\"><img title=\"Edit\" alt=\"Edit\" src=\"".APPLICATION_URL."/images/edit.png\" border=\"0\" /></a>'";			
			$json .= ",'<a href=\"".APPLICATION_URL."Zone/del/id/".$row['country_id']."\"><img title=\"Delete\" alt=\"Delete\" src=\"".APPLICATION_URL."/images/deleteicon.png\" border=\"0\" /></a>'";			
			$json .= "]}";
			$rc = true;
			$i++;
		}
